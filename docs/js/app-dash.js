...
document.getElementById('form-termo').addEventListener('submit',async e=>{
  ...
  try{
    let dup = null;
    try { dup = await checkDuplicateTermo(); } catch(e){}
    if (dup) {
    ...
    if(issues){
      const ac=document.getElementById('termo-accion').value;
      if(!ac){
        setLoading('btn-termo-submit','spinner-termo','btn-termo-text',false);
...
document.getElementById('form-centrifugas').addEventListener('submit',async e=>{
  ...
  try{
    let dup = null;
    try { dup = await checkDuplicateCentrifugas(); } catch(e){}
    if (dup) {
...
document.getElementById('form-mesones').addEventListener('submit',async e=>{
  ...
  try{
    let dup = null;
    try { dup = await checkDuplicateMesones(); } catch(e){}
    if (dup) {
...
document.getElementById('form-refri-temp').addEventListener('submit',async e=>{
  ...
  try{
    ...
    if(r.success){
      addDatalistOption('list-refri-obs', obsVal);showToast('✅ '+r.message);e.target.reset();document.getElementById('refri-fecha').value=today();resetRangoRefri();autoSetAmPm();if(typeof clearRecordsMonthCache==='function')clearRecordsMonthCache();prefetchDashboard();
...
document.getElementById('form-conductividad').addEventListener('submit',async e=>{
  ...
  try{
    ...
    if(r.success){
      addDatalistOption('list-conduct-obs', obsVal);showToast('✅ '+r.message);e.target.reset();document.getElementById('conduct-fecha').value=today();resetRangoConductividad();autoSetAmPm();if(typeof clearRecordsMonthCache==='function')clearRecordsMonthCache();prefetchDashboard();
...
function renderDailyView(reg){
  ...
  if(isModuloActivo('elim-muestras')){
    ...
    const countText = elimHoy.length === 1 
      ? `1 sector (${elimHoy[0].sector || getInicialesResponsable(elimHoy[0].responsable)})` 
      : `${elimHoy.length} registros`;
    html+=`<div class="status-item ${doneElim?'done':'miss'}"><span class="status-dot ${doneElim?'green':'red'}"></span>${doneElim?'Registrado hoy ('+countText+')':'Pendiente del día'}</div>`;
    html+='</div></div>';
  }
...
function initRevAdminSelectors(){
  ...
  if(typeof populateChips==='function'&&typeof ALL_REV_TYPES!=='undefined'){
    populateChips('rev-chips', ALL_REV_TYPES);
  }
  loadRevStatus();
}
...
// QR
const QR_TABS=['areas','salas','centrifugas','refrigeradores','refri-limpieza','conductividad','etiquetadoras','cobas','dxh900','elim-muestras'];
function switchQrTab(tab){
  ...
  const btnPrintAll=document.getElementById('btn-print-all-elim');
  if(btnPrintAll) btnPrintAll.style.display = (tab === 'elim-muestras') ? 'inline-flex' : 'none';
  // Reset QR display
  ...
  if(tab==='conductividad'){
    ...
    lbl.textContent='Selecciona Sector / Tipo de Muestra';
    const sectorList = (state.elimSectores && state.elimSectores.length) ? state.elimSectores : ELIM_SECTORES_DEF;
    populateSelect('admin-select', sectorList.map(s => s.nombre), '— Seleccionar Sector —');
  } else {
    selGroup.style.display='';
...
function generateQR(){
  ...
  if(state.qrTab==='conductividad'){
    ...
    if(!rawVal)return;
    url=`${base}?modulo=elim-muestras&sector=${encodeURIComponent(rawVal)}`;
    val=`Eliminación Muestras - ${rawVal}`;
  } else {
    const rawVal=document.getElementById('admin-select').value;
    if(!rawVal)return;
...
function printLabel50x30() {
  ...
  if (state.qrTab === 'etiquetadoras') {
    ...
    category = 'Eliminación Muestras';
    const rawVal = document.getElementById('admin-select').value;
    details = rawVal;
    const sectorList = (state.elimSectores && state.elimSectores.length) ? state.elimSectores : ELIM_SECTORES_DEF;
    const s = sectorList.find(x => x.nombre === rawVal);
    if (s && s.almacenamiento) {
      details2 = s.almacenamiento;
    }
  } else {
  ...
  const isElim = (state.qrTab === 'elim-muestras');
  ...
  if (isElim) {
    const slblDetailsLines = wrapText(slblDetails, 19);
    slbl += `A190,92,0,2,1,1,N,"${slblDetailsLines[0]}"#10#13`;
    if (slblDetailsLines[1]) {
      slbl += `A190,116,0,2,1,1,N,"${slblDetailsLines[1].substring(0, 20)}"#10#13`;
      if (slblDetails2Lines[0]) {
        slbl += `A190,140,0,1,1,1,N,"${slblDetails2Lines[0].substring(0, 22)}"#10#13`;
      }
    } else {
      if (slblDetails2Lines[0]) {
...
<head>
  ...
  <style>
    ...
    * {
    ...
    .details {
      font-size: ${isElim ? (details.length > 30 ? '6.5pt' : '7.5pt') : '7.5pt'};
      font-weight: 700;
      line-height: 1.1;
      margin: 0 0 0.5mm 0;
      color: #000;
      ...
      display: -webkit-box;
      -webkit-line-clamp: ${isElim ? '2' : (details2 ? '1' : '2')};
      -webkit-box-orient: vertical;
      overflow: hidden;
      width: 100%;
    }
    .details-secondary {
      font-size: ${isElim ? '5.5pt' : '6.5pt'};
      font-weight: 600;
      line-height: 1.1;
      margin: 0 0 1mm 0;
      color: #444;
...
function printAllElimLabels50x30() {
  ...
  if (!sectorList || !sectorList.length) {
    showToast('⚠️ No hay sectores configurados para imprimir', 'error');
    return;
  }
  ...
  const generateQrDataUrl = (text) => {
    try {
      ...
      console.warn('Fallback a qrserver para URL:', text, e);
    }
    return `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(text)}`;
  };

  const pagesHtml = sectorList.map(s => {
    const url = `${base}?modulo=elim-muestras&sector=${encodeURIComponent(s.nombre)}`;
    const qrImg = generateQrDataUrl(url);
    const cleanCategory = escapeHtml('Eliminación Muestras');
    const cleanDetails = escapeHtml(s.nombre);
    const storageNote = s.almacenamiento ? escapeHtml(s.almacenamiento) : '';
    const detailsFontSize = s.nombre.length > 30 ? '6.5pt' : '7.5pt';
    ...
    return `
      <div class="label-page">
        ...
        <div class="text-container">
          <div class="title">Lab. Clínico HRT</div>
          <div class="category">${cleanCategory}</div>
          <div class="details" style="font-size:${detailsFontSize};">${cleanDetails}</div>
          ${storageNote ? `<div class="details-secondary">${storageNote}</div>` : ''}
          <div class="footer">Escanear para registrar</div>
        </div>
      </div>
    `;
...
<html>
<head>
  <title>Etiquetas 50x30 - Eliminación de Muestras (Todos los Sectores)</title>
  <style>
    @page {
      size: 50mm 30mm;
      margin: 0;
...
async function init(){
  ...
  // 1. Procesar parámetros de URL / QR de inmediato (0 ms) para navegar a la sección requerida sin esperar la red
  try { checkUrlParams(); } catch(e){}

  // 2. Cargar maestros (0 ms instantáneo si viene inyectado del servidor)
  await loadMaestros();
  enforceMaxDateInputs();
  // Re-evaluar parámetros QR para enlazar chips/selects recién poblados
  try { checkUrlParams(); } catch(e){}
...
function parseAlmacenamientoToOffset(almacenamiento) {
  if (!almacenamiento) return null;
  const str = almacenamiento.toLowerCase();
  if (str.includes('mes')) {
    const m = str.match(/(\d+)\s*mes/);
    return { meses: m ? parseInt(m[1], 10) : 1 };
  }
  if (str.includes('semana')) {
    const m = str.match(/(\d+)\s*semana/);
    const semanas = m ? parseInt(m[1], 10) : 1;
    return { dias: semanas * 7 };
  }
  if (str.includes('hora')) {
    const m = str.match(/(\d+)\s*hora/);
    const horas = m ? parseInt(m[1], 10) : 24;
    return { dias: Math.max(1, Math.round(horas / 24)) };
  }
  if (str.includes('día') || str.includes('dia')) {
    const m = str.match(/(\d+)\s*d[íi]a/);
    return { dias: m ? parseInt(m[1], 10) : 1 };
  }
  return null;
}

function calcularFechaCorte(fechaBaseStr, sectorDef) {
  if (!fechaBaseStr || !sectorDef) return '';
  const parts = fechaBaseStr.split('-').map(Number);
  if (parts.length !== 3 || isNaN(parts[0]) || isNaN(parts[1]) || isNaN(parts[2])) return '';
  
  let offset = null;
  if (typeof sectorDef === 'object') {
    if (sectorDef.meses || sectorDef.dias) {
      offset = { meses: sectorDef.meses || 0, dias: sectorDef.dias || 0 };
    } else if (sectorDef.almacenamiento) {
      offset = parseAlmacenamientoToOffset(sectorDef.almacenamiento);
    }
  } else if (typeof sectorDef === 'string') {
    const sectorList = (state.elimSectores && state.elimSectores.length) ? state.elimSectores : ELIM_SECTORES_DEF;
    const found = sectorList.find(s => s.nombre === sectorDef);
    if (found) {
      offset = { meses: found.meses || 0, dias: found.dias || 0 };
      if (!offset.meses && !offset.dias && found.almacenamiento) {
        offset = parseAlmacenamientoToOffset(found.almacenamiento);
      }
    } else {
      offset = parseAlmacenamientoToOffset(sectorDef);
    }
  }

  if (!offset) return '';

  // Use noon (12:00) to be completely immune to DST midnight transitions
  const dt = new Date(parts[0], parts[1] - 1, parts[2], 12, 0, 0);
  if (offset.meses) {
    const origDay = dt.getDate();
    dt.setMonth(dt.getMonth() - offset.meses);
    if (dt.getDate() !== origDay) {
      dt.setDate(0); // overflow correction
    }
  }
  if (offset.dias) {
    dt.setDate(dt.getDate() - offset.dias);
  }
...
function onElimSectorChange() {
  const sectorSel = document.getElementById('elim-sector');
  const infoEl = document.getElementById('elim-almacenamiento-info');
  const descEl = document.getElementById('elim-almacenamiento-desc');
  const fechaInput = document.getElementById('elim-fecha');
  const corteInput = document.getElementById('elim-fecha-corte');
  const textInput = document.getElementById('elim-muestras-texto');
  ...
  if (!val) {
    ...
    if (typeof checkDuplicateElimMuestras === 'function') {
  ...
  const sectorList = (state.elimSectores && state.elimSectores.length) ? state.elimSectores : ELIM_SECTORES_DEF;
  const found = sectorList.find(s => s.nombre === val);

  if (found) {
    if (infoEl && descEl) {
      descEl.textContent = found.almacenamiento + (found.nota ? ` (${found.nota})` : '');
      infoEl.style.display = 'block';
    }
    const fechaBase = (fechaInput && fechaInput.value) ? fechaInput.value : today();
    const proposedCutoff = calcularFechaCorte(fechaBase, found);
    if (corteInput && proposedCutoff) {
      corteInput.value = proposedCutoff;
...
function onElimFechaChange() {
  const sectorSel = document.getElementById('elim-sector');
  const corteInput = document.getElementById('elim-fecha-corte');
  const fechaInput = document.getElementById('elim-fecha');
  if (sectorSel && sectorSel.value) {
    const sectorList = (state.elimSectores && state.elimSectores.length) ? state.elimSectores : ELIM_SECTORES_DEF;
    const found = sectorList.find(s => s.nombre === sectorSel.value);
    if (found && fechaInput && fechaInput.value) {
      const proposedCutoff = calcularFechaCorte(fechaInput.value, found);
      if (corteInput && proposedCutoff) {
        corteInput.value = proposedCutoff;
        updateMuestrasEliminadasText();
      }
...
async function loadElimMuestrasHistorialForm() {
  ...
  if (sectorSel && sectorSel.options.length <= 1) {
    ...
    sectorSel.innerHTML = '<option value="">— Seleccionar Sector —</option>' + sectorList.map(s => `<option value="${escapeHtml(s.nombre)}">${escapeHtml(s.nombre)}</option>`).join('');
    if (currentVal) sectorSel.value = currentVal;
  }

  if (document.getElementById('elim-fecha') && !document.getElementById('elim-fecha').value) {
    document.getElementById('elim-fecha').value = today();
  }

  if (sectorSel && sectorSel.value) {
    onElimSectorChange();
  } else if (document.getElementById('elim-fecha-corte')) {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      ...
      list.innerHTML = data.map(r => `
        <div style="padding:10px; border-bottom:1px solid rgba(255,255,255,0.04); font-size:13px; line-height:1.4;">
          <div style="display:flex; justify-content:space-between; margin-bottom:4px; font-weight:700;">
            <span style="color:var(--primary-color);">🗑️ ${r.sector ? `<strong>${escapeHtml(r.sector)}</strong>: ` : ''}${escapeHtml(r.muestras_eliminadas)}</span>
...
async function saveElimMuestrasForm(e) {
  ...
  const sectorInput = document.getElementById('elim-sector');
  const textInput = document.getElementById('elim-muestras-texto');
  const fechaInput = document.getElementById('elim-fecha');
  ...
  if (!sectorVal) {
    showToast('Debe seleccionar un sector / tipo de muestra.', 'error');
    return;
  }
  ...
  const corteInput = document.getElementById('elim-fecha-corte');
  if (corteInput && fechaInput && corteInput.value && fechaInput.value && corteInput.value > fechaInput.value) {
    showToast('La fecha de corte no puede ser posterior a la fecha de eliminación.', 'error');
    return;
  }
  ...
  let dup = null;
  try { dup = await checkDuplicateElimMuestras(); } catch(e){}
  if (dup) {
  ...
  try {
    ...
    if (res.success) {
      ...
      const prefillIndicator = document.getElementById('elim-sector-prefill-indicator');
      const isPrefilled = prefillIndicator && prefillIndicator.style.display !== 'none';
      if (!isPrefilled) {
...
async function checkDuplicateTermo() {
  ...
  if (typeof recentTermoCache !== 'undefined' && Array.isArray(recentTermoCache) && recentTermoCache.length > 0) {
    const foundRecent = recentTermoCache.find(r =>
      parseInt(r.dia) === dia &&
      parseInt(r.mes) === mes &&
    ...
    if (foundRecent) {
      const respUser = foundRecent.responsable || 'Desconocido';
...
async function checkDuplicateCentrifugas() {
  ...
  if (typeof recentCentrifugasCache !== 'undefined' && Array.isArray(recentCentrifugasCache) && recentCentrifugasCache.length > 0) {
    const duplicates = [];
    const currentTipo = String(tipoVal || 'Diaria').trim().toLowerCase();
    selChips.forEach(chipName => {
      const found = recentCentrifugasCache.find(r => {
        ...
        if (currentTipo === 'diaria') return rTipo === 'diaria' || rTipo === 'semanal';
        if (currentTipo === 'semanal') return rTipo === 'semanal';
    ...
    if (duplicates.length > 0) {
      ...
      return {
        duplicates,
        cleanMessage: `Se detectó registro previo de mantenimiento de centrífuga para la fecha <strong>${fechaFmt}</strong>:`,
        message: `Centrífuga(s) ya registradas hoy: ${duplicates.map(d=>d.name).join(', ')}.`
      };
    }
  }
...
async function checkDuplicateMesones() {
  ...
  if (typeof recentMesonesCache !== 'undefined' && Array.isArray(recentMesonesCache) && recentMesonesCache.length > 0) {
    const duplicates = [];
    selChips.forEach(chipName => {
      const found = recentMesonesCache.find(r => 
        parseInt(r.dia) === dia &&
        parseInt(r.mes) === mes &&
        parseInt(r.anio) === anio &&
    ...
    if (duplicates.length > 0) {
      ...
      return {
        duplicates,
        cleanMessage: duplicates.length > 1 
...
async function checkDuplicateElimMuestras() {
  ...
  const sectorVal = document.getElementById('elim-sector') ? document.getElementById('elim-sector').value.trim() : '';
  if (!sectorVal) {
    alertEl.classList.remove('visible');
    alertEl.style.display = 'none';
    alertEl.innerHTML = '';
  ...
  const existing = regs.elimMuestras.find(r => 
    ...
    parseInt(r.mes) === mes &&
    parseInt(r.anio) === anio &&
    ((r.sector || '').trim().toLowerCase() === sectorVal.toLowerCase())
  );

  if (existing) {
    const respUser = existing.responsable || 'Desconocido';
    const fechaFmt = `${String(dia).padStart(2,'0')}/${String(mes).padStart(2,'0')}/${anio}`;
    const sectorTxt = existing.sector ? ` para <strong>${escapeHtml(existing.sector)}</strong>` : '';
    const sectorTxtPlain = existing.sector ? ` (${existing.sector})` : '';
    const htmlMsg = `⚠️ <strong>Aviso de Duplicidad:</strong> El registro de eliminación de muestras${sectorTxt} para el día <strong>${fechaFmt}</strong> ya fue realizado por <strong>"${escapeHtml(respUser)}"</strong>.<br>¿Está seguro que desea continuar?`;
    alertEl.innerHTML = htmlMsg;
    alertEl.classList.add('visible');
    alertEl.style.display = 'block';
    ...
    return {
      existing,
      cleanMessage: `Se detectó un registro previo de eliminación de muestras${sectorTxt} para la fecha <strong>${fechaFmt}</strong>:`,
      message: `El registro de eliminación de muestras${sectorTxtPlain} para el día ${fechaFmt} ya fue realizado por "${respUser}".`
    };
  } else {
    alertEl.classList.remove('visible');
    alertEl.style.display = 'none';
...
function initDuplicateCheckListeners() {
  ...
  const elimSector = document.getElementById('elim-sector');
  if (elimFecha) { elimFecha.addEventListener('change', checkDuplicateElimMuestras); elimFecha.addEventListener('input', debouncedElim); }
  if (elimSector) { elimSector.addEventListener('change', checkDuplicateElimMuestras); }
}
...
function openEditTermoModal(index) {
  ...
  document.getElementById('edit-termo-resp').value = getInicialesResponsable(rec.responsable);
...
function openEditCentrifugaModal(idx) {
  ...
  document.getElementById('edit-cent-resp').value = getInicialesResponsable(rec.responsable);
...
function openEditMesonModal(idx) {
  ...
  document.getElementById('edit-meson-resp').value = getInicialesResponsable(rec.responsable);
...
function openEditRefriTempModal(idx) {
  ...
  document.getElementById('edit-refri-resp').value = getInicialesResponsable(rec.responsable);
...
function openEditLimpRefriModal(idx) {
  ...
  document.getElementById('edit-limp-refri-resp').value = getInicialesResponsable(rec.responsable);
...
function openEditConductividadModal(idx) {
  ...
  document.getElementById('edit-conduct-resp').value = getInicialesResponsable(rec.responsable);
...
function openEditCobasModal(idx) {
  ...
  document.getElementById('edit-cobas-resp').value = getInicialesResponsable(rec.responsable);
