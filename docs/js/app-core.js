...
function populateChips(id,items){const c=document.getElementById(id);c.innerHTML=items.map(i=>`<label class="chip-item" onclick="toggleChip(this)"><input type="checkbox" value="${i}"/>${i}</label>`).join('')}
function toggleChip(el){const cb=el.querySelector('input');cb.checked=!cb.checked;el.classList.toggle('selected',cb.checked)}
function centNum(name){const m=name.match(/(\d+)/);return m?parseInt(m[1]):0}
function getSelectedChips(id){return Array.from(document.querySelectorAll(`#${id} input:checked`)).map(i=>i.value)}
function toggleGrupoPreanalisis(){const btn=document.getElementById('btn-grupo-preanalisis');const active=btn.classList.toggle('active');const chips=document.querySelectorAll('#cent-chips .chip-item');chips.forEach(c=>{const v=c.querySelector('input').value;const num=centNum(v);const isP=PREANALISIS.includes(num);if(isP){if(active){c.classList.add('selected');c.querySelector('input').checked=true}else{c.classList.remove('selected');c.querySelector('input').checked=false}}})}

// URL params
function checkUrlParams(){const p=new URLSearchParams(window.location.search);const area=p.get('area'),sala=p.get('sala'),cent=p.get('centrifuga'),grupo=p.get('grupo');if(area){document.getElementById('area-prefill-name').textContent=area;document.getElementById('area-prefill-indicator').style.display='block';const w=setInterval(()=>{const s=document.getElementById('termo-area');if(Array.from(s.options).find(o=>o.value===area)){s.value=area;clearInterval(w)}},300);navigateTo('termo')}if(sala){const w=setInterval(()=>{const chips=document.querySelectorAll('#meson-chips .chip-item');if(chips.length){chips.forEach(c=>{if(c.querySelector('input').value===sala){c.classList.add('selected');c.querySelector('input').checked=true}});clearInterval(w)}},300);navigateTo('mesones')}if(cent){const w=setInterval(()=>{const chips=document.querySelectorAll('#cent-chips .chip-item');if(chips.length){chips.forEach(c=>{if(c.querySelector('input').value===cent){c.classList.add('selected');c.querySelector('input').checked=true}});clearInterval(w)}},300);navigateTo('centrifugas')}if(grupo==='preanalisis'){const w=setInterval(()=>{const chips=document.querySelectorAll('#cent-chips .chip-item');if(chips.length){document.getElementById('btn-grupo-preanalisis').classList.add('active');chips.forEach(c=>{const v=c.querySelector('input').value;if(PREANALISIS.includes(centNum(v))){c.classList.add('selected');c.querySelector('input').checked=true}});clearInterval(w)}},300);navigateTo('centrifugas')}}
